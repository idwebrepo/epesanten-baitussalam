...
class Report_set extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if ($this->session->userdata('logged') == NULL) {
			header("Location:" . site_url('manage/auth/login') . "?locyation=" . urlencode($_SERVER['REQUEST_URI']));
		}
	...
	public function cari_report_period()
	{
		...
		$dataPeriod     = $this->db->query("SELECT period_start, period_end FROM period WHERE period_id = '$id_periode'")->row_array();
		$dataPayment    = $this->db->query("SELECT pos_name, payment_type, payment_id FROM `payment` JOIN pos ON payment.pos_pos_id = pos.pos_id JOIN account ON account.account_id = pos.account_account_id WHERE payment.period_period_id = '$id_periode' AND account.account_majors_id = '$id_majors' ORDER BY payment.payment_type DESC")->result_array();
		...
		$bulPayment = $this->Report_model->get_period_bulan($id_periode, $id_kelas, $ds, $de);
		$bebPayment = $this->Report_model->get_period_bebas($id_periode, $id_kelas, $ds, $de);

		foreach ($bulPayment as $result) {
			$bulPayData[] = $result;
		}


		foreach ($bebPayment as $result) {
			$bebPayData[] = $result;
		}


		foreach ($paymentData as $result) {
			echo '<h4><strong>' . $result['pos_name'] . '</strong></h4>';

			if ($result['payment_type'] == "BULAN") {

				echo '<div class="box-body table-responsive"><table class="table table-responsive table-hover table-bordered" style="white-space: nowrap;">
					<tr>
						<th>No.</th>
						<th>Tanggal</th>
						<th>NIS</th>
				...
				$sumBulan = 0;
				$no = 1;
				foreach ($bulPayData as $row) {

					if ($result['payment_id'] == $row['payment_id']) {
	...
	public function report_period_print()
	{
		...
		$file_pdf = 'Laporan Pembayaran Kelas ' . $dataClass['class_name'] . ' T.A.' . $dataPeriod['period_start'] . '/' . $dataPeriod['period_end'] . ' Tanggal ' . pretty_date($ds, "d/m/Y", FALSE) . ' Sampai ' . pretty_date($de, "d/m/Y", FALSE);
	...
	public function report_cetak_bulan()
	{
		...
		$html = $this->load->view('report/report_cetak_bulan', $data, TRUE);
	...
	public function report_cetak_bebas()
	{
		...
		$file_pdf = 'Laporan_Pembayaran_' . $dHead->pos_name . '- TA' . $dHead->period_start . '-' . $dHead->period_end . '_Kelas_' . $dataKelas . '_' . pretty_date(date('Y-m-d'), 'd F Y', false);
	...
	public function report_mutasi()
	{
		...
		$data['main'] = 'report/report_mutasi_list';
		$this->load->view('manage/layout', $data);
	}

	public function search_report_mutasi()
	{
		...
		if ($majors_id != 'all') {
			...
			$kas_account_id  = implode(',', $this->input->post('kas_account_id'));
		}
		...
		if (isset($kas_account_id) && !empty($kas_account_id) && $kas_account_id != '') {
			$params['kas_account_id'] = $kas_account_id;
			$param['kas_account_id'] = $kas_account_id;
		}
		...
		$bulan  = $this->Bulan_model->list_mutasi($params);
		$free   = $this->Bebas_pay_model->list_mutasi($params);
		$debit  = $this->Debit_model->list_mutasi($params);
		$kredit = $this->Kredit_model->list_mutasi($params);
		// $gaji   = $this->Kredit_model->gaji_mutasi($params);

		$gaji   = array();

		$bulanLast  = $this->Bulan_model->get_last_mutasi($param);
		$freeLast   = $this->Bebas_pay_model->get_last_mutasi($param);
		$debitLast  = $this->Debit_model->get_last_mutasi($param);
		$kreditLast = $this->Kredit_model->get_last_mutasi($param);
		// $gajiLast 	= $this->Kredit_model->gaji_last_mutasi($param);
		...
		foreach ($gajiLast as $row) {
			if (is_array($row) && array_key_exists('kredit_value', $row)) {
		...
		echo '<tr>
									<td></td>
									<td></td>
									<td>Saldo Sebelum Tanggal ' . pretty_date($ds, 'd/m/Y', FALSE) . '</td>
									<td></td>
									<td></td>
									<td> - </td>
									<td> - </td>
		...
		foreach ($bulan as $row) {
			echo '
    				              <tr>
    				                <td>' . pretty_date($row['tanggal'], 'Ymd', FALSE) . '</td>
    				                <td>' . pretty_date($row['tanggal'], 'd/m/Y', FALSE) . '</td>
    				                <td>' . $row['nama_kas'] . '</td>
		...
		foreach ($gaji as $row) {
			echo '<tr>
    				                ...
    				                <td> - </td>
    				                <td>' . 'Rp ' . number_format($row['kredit_total'], 0, ",", ".") . '</td>
    				                <td> - </td>
    						    </tr>';
			$sumGaji += $row['kredit_total'];
		}
		$sumKredit = 0;
		...
		echo '</table>
    					...
    					<table class="table">
        			        <tr>
        					    <td>
                					<div class="md-6">
                					    <a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_detail_mutasi/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Per Jenis Anggaran
                					    </a>
                					    <a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_detail_mutasi/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Per Jenis Anggaran
                					    </a>
                					</div>
        					    </td>
        					    <td>
        					        <div class="pull-right">
                					    <a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_rekap_mutasi/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Rekap Laporan
                					    </a>
                					    <a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_rekap_mutasi/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Rekap Laporan
                					    </a>
                					</div>
        					    </td>
    					    </tr>
	...
	public function search_report_jurnal()
	{
		...
		if (isset($majors_id) && !empty($majors_id) && $majors_id != 'all') {
			$params['account_majors_id'] = $majors_id;
			$param['account_majors_id'] = $majors_id;
			// $akunKas = $this->db->query("SELECT account_id FROM account WHERE account.account_category = '2' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Tunai%' AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->result_array();
			$akunKas = $this->db->query("SELECT account_id FROM account WHERE account.account_category = '2' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%'  AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->result_array();

			$saldoAwal = $this->db->query("SELECT SUM(saldo_awal.saldo_awal_debit) AS nominal FROM `saldo_awal` JOIN account ON saldo_awal.saldo_awal_account =account.account_id WHERE account.account_category = '2' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%'  AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->row_array();
		} else if (isset($majors_id) && !empty($majors_id) && $majors_id == 'all') {
			$params['kas_noref'] = 'isi';
			$param['kas_noref'] = 'isi';

			// $akunKas = $this->db->query("SELECT account_id FROM account WHERE account.account_category = '2' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Tunai%' AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->result_array();
			$akunKas = $this->db->query("SELECT account_id FROM account WHERE account.account_category = '2' AND account.account_code LIKE '1%' AND  account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->result_array();

			// $saldoAwal = $this->db->query("SELECT SUM(saldo_awal.saldo_awal_debit) AS nominal FROM `saldo_awal` JOIN account ON saldo_awal.saldo_awal_account =account.account_id WHERE account.account_category = '2' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Tunai%' AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->row_array();
			$saldoAwal = $this->db->query("SELECT SUM(saldo_awal.saldo_awal_debit) AS nominal FROM `saldo_awal` JOIN account ON saldo_awal.saldo_awal_account =account.account_id WHERE account.account_category = '2' AND account.account_code LIKE '1%' AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->row_array();
		}

		$kasAkun = array();

		foreach ($akunKas as $ak):
			$kasAkun[] = $ak['account_id'];
		endforeach;
		// $kasAkun[] = 300;

		$params['kas_account_id'] = $kasAkun;
		$param['kas_account_id'] = $kasAkun;
		...
		echo '</table>
    					...
    					<table class="table">
        			        <tr>
        					    <td>
                					<div class="md-6">
                					    <a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_detail_jurnum/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Per Jenis Anggaran
                					    </a>
                					    <a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_detail_jurnum/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Per Jenis Anggaran
                					    </a>
                					</div>
        					    </td>
        					    <td>
        					        <div class="pull-right">
                					    <a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_rekap_jurnum/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Rekap Laporan
                					    </a>
                					    <a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_rekap_jurnum/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Rekap Laporan
                					    </a>
                					</div>
        					    </td>
    					    </tr>
	...
	public function search_report_date()
	{
		...
		if ($start >= '7') {
			$yearstart 	= date("Y", strtotime($ds));
		} else {
			$yearstart 	= date("Y", strtotime($ds)) - 1;
		}

		if ($end >= '7') {
			$yearend	= date("Y", strtotime($de)) + 1;
		} else {
			$yearend	= date("Y", strtotime($de));
		}

		$period = $this->db->query("SELECT period_id FROM period WHERE period_start = $yearstart AND period_end = $yearend;")->row_array();
		...
		if ($period['period_id'] != '') {
			...
			echo '<tr style="background-color: #F0B2B2;">
										<td colspan = "7" align = "right"><strong>Saldo Awal</strong></td>
										<td>' . 'Rp ' . number_format($sumLastA, 0, ",", ".") . '</td>
										<td>' . 'Rp ' . number_format($sumLastB, 0, ",", ".") . '</td>
									</tr>';
			echo '<tr style="background-color: #F0B2B2;">
										<td colspan = "7" align = "right"><strong>Total Saldo Awal</strong></td>
										<td colspan = "2" align = "center">' . 'Rp ' . number_format(($saldoAwal['nominal'] + $sumLastA) - $sumLastB, 0, ",", ".") . '</td>
									</tr>';
			echo '<tr style="background-color: #FFFCBE;">
										<td colspan = "7" align = "right"><strong>Total (Sub Total + Saldo Awal)</strong></td>
			...
			echo '<tr style="background-color: #c2d2f6;">
										<td colspan = "7" align = "right"><strong>Saldo Akhir</strong></td>
										<td colspan = "2" align = "center">' . 'Rp ' . number_format(($saldoAwal['nominal'] + $sumLastA + $sumA) - ($sumLastB + $sumB), 0, ",", ".") . '</td>
									</tr>';
			echo '</table>
							</div>
							<div class="box-footer">
							<table class="table">
								<tr>
									<td>
										<div class="md-6">
											<a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_detail_kas/' . $ds . '/' . $de . '/' . $period_id . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Per Jenis Anggaran
											</a>
											<a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_detail_kas/' . $ds . '/' . $de . '/' . $period_id . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Per Jenis Anggaran
											</a>
										</div>
									</td>
									<td>
										<div class="pull-right">
											<a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_rekap_kas/' . $ds . '/' . $de . '/' . $period_id . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Rekap Laporan
											</a>
											<a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_rekap_kas/' . $ds . '/' . $de . '/' . $period_id . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Rekap Laporan
											</a>
										</div>
									</td>
								</tr>
			...
			echo "<script>";
			echo "alert('Bulan ajaran dimulai dari bulan Juli sampai Juni. Silahkan sesuaikan lagi pencarian data anda.');";
			echo "</script>";
		}
	}
	...
	public function search_report_bank()
	{
		...
		if ($period['period_id'] != '') {
			...
			if (isset($majors_id) && !empty($majors_id) && $majors_id != 'all') {
				...
				$saldoAwal = $this->db->query(
					"SELECT SUM(saldo_awal.saldo_awal_debit) AS nominal FROM `saldo_awal` 
					JOIN account ON saldo_awal.saldo_awal_account =account.account_id 
					WHERE account.account_category = '2' 
					AND account.account_code LIKE '1%' 
					AND account.account_description LIKE '%Bank%' 
					AND account.account_note IN (
						SELECT account_id FROM account 
						WHERE account.account_category = '0' 
						AND account.account_code LIKE '1%' 
						AND account.account_description LIKE '%Aktiva%')"
				)->row_array();
			}
			...
			// </tbody>
			// <tr style="background-color: #E2F7FF;">
			// <td colspan = "7" align = "right"><strong>Sub bulan</strong></td>
			// <td>'.'Rp '.number_format($sumBulan,0,",",".").'</td>
			// </tr>
			// <tr style="background-color: #E2F7FF;">
			// 	<td colspan = "7" align = "right"><strong>Sub bebas</strong></td>
			// 	<td>'.'Rp '.number_format($sumFree,0,",",".").'</td>
			// </tr>
			// <tr style="background-color: #E2F7FF;">
			// 	<td colspan = "7" align = "right"><strong>Sub debit</strong></td>
			// 	<td>'.'Rp '.number_format($sumDebit,0,",",".").'</td>
			// </tr>
			// <tr style="background-color: #E2F7FF;">
			// 	<td colspan = "7" align = "right"><strong>Sub Kredit</strong></td>
			// 	<td>'.'Rp '.number_format($sumKredit,0,",",".").'</td>
			// </tr>
			// <tr style="background-color: #E2F7FF;">
			// 	<td colspan = "7" align = "right"><strong>Sub Gaji</strong></td>
			// 	<td>'.'Rp '.number_format($sumGaji,0,",",".").'</td>
			// </tr>
			// ';

			// echo '<tr style="background-color: #F0B2B2;">
			// 		<td colspan = "7" align = "right"><strong>Saldo bulan akhir</strong></td>
			// 		<td>'.'Rp '.number_format($sumBulanLast,0,",",".").'</td>
			// 	</tr>';
			// echo '<tr style="background-color: #F0B2B2;">
			// 		<td colspan = "7" align = "right"><strong>Saldo bebas akhir</strong></td>
			// 		<td>'.'Rp '.number_format($sumFreeLast,0,",",".").'</td>
			// 	</tr>';
			// echo '<tr style="background-color: #F0B2B2;">
			// 		<td colspan = "7" align = "right"><strong>Saldo Debit akhir</strong></td>
			// 		<td>'.'Rp '.number_format($sumDebitLast,0,",",".").'</td>
			// 	</tr>';
			// echo '<tr style="background-color: #F0B2B2;">
			// 		<td colspan = "7" align = "right"><strong>Saldo kredit akhir</strong></td>
			// 		<td>'.'Rp '.number_format($sumKreditLast,0,",",".").'</td>
			// 	</tr>';
			// echo '<tr style="background-color: #F0B2B2;">
			// 		<td colspan = "7" align = "right"><strong>Saldo gaji akhir</strong></td>
			// 		<td>'.'Rp '.number_format($sumGajiLast,0,",",".").'</td>
			// 	</tr>';
			...
			echo '</table>
							...
							<table class="table">
								<tr>
									<td>
										<div class="md-6">
											<a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_detail_bank/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Per Jenis Anggaran
											</a>
											<a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_detail_bank/' . $ds . '/' . $de . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Per Jenis Anggaran
											</a>
										</div>
									</td>
									<td>
										<div class="pull-right">
											<a class="btn btn-danger" target="_blank" href="' . base_url() . 'manage/report/cetak_rekap_bank/' . $ds . '/' . $de . '/' . '/' . $majors_id . '"><span class="fa fa-file-pdf-o"></span> PDF Rekap Laporan
											</a>
											<a class="btn btn-success" target="_blank" href="' . base_url() . 'manage/report/excel_rekap_bank/' . $ds . '/' . $de . '/'  . '/' . $majors_id . '"><span class="fa fa-file-excel-o"></span> Excel Rekap Laporan
											</a>
										</div>
									</td>
								</tr>
	...
	public function cetak_detail_jurnum()
	{
		//ob_start();
		...
		if (isset($majors_id) && !empty($majors_id) && $majors_id != 'all') {
			...
			$akunKas = $this->db->query("SELECT account_id FROM account WHERE account.account_category = '2' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%' AND (account.account_description LIKE '%Tunai%' OR account.account_description LIKE '%Bank%' OR account.account_description LIKE '%Flip%' ) AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_majors_id = '$majors_id' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->result_array();
			...
			$akunKas = $this->db->query("SELECT account_id FROM account WHERE account.account_category = '2' AND account.account_code LIKE '1%' AND (account.account_description LIKE '%Tunai%' OR account.account_description LIKE '%Bank%' OR account.account_description LIKE '%Flip%' ) AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' AND account.account_code LIKE '1%' AND account.account_description LIKE '%Aktiva%')")->result_array();
		...
		$data['bulan']  = $this->Report_model->get_rekap_bulan_jurnum($params);
		$data['free']   = $this->Report_model->get_rekap_bebas_jurnum($params);
		$data['debit']  = $this->Report_model->get_rekap_debit_jurnum($params);
		$data['kredit'] = $this->Report_model->get_rekap_kredit_jurnum($params);
		$data['gaji']   = $this->Report_model->get_rekap_gaji_jurnum($params);
		...
		// $data['bulanLast']  = $this->Report_model->get_rekap_bulan_last_jurnum($param);

		$data['bulanLast']  = $this->Report_model->get_last_jurnal($param);
		$data['freeLast']   = $this->Report_model->get_rekap_bebas_last_jurnum($param);
		$data['debitLast']  = $this->Report_model->get_rekap_debit_last_jurnum($param);
		$data['kreditLast'] = $this->Report_model->get_rekap_kredit_last_jurnum($param);
		$data['gajiLast']   = $this->Report_model->get_rekap_gaji_last_jurnum($param);

		// $data['bulanLast']  = array();
		// $data['freeLast']  = array();
		// $data['debitLast']  = array();
		// $data['kreditLast'] = array();
		// $data['gajiLast']   = array();
		...
		// echo $filename;
		// exit;
		// $this->load->view('report/cetak_detail_jurnum', $data);
		$html = $this->load->view('report/cetak_detail_jurnum', $data, TRUE);

		$paper = 'A4';
		$orientation = 'P';
		$stream = TRUE;

		// $this->dompdflib->generate($html, $filename, $paper, $orientation, $stream);
		$this->dompdflib->download($html, $filename, $paper, $orientation, $stream);
	}

	public function excel_detail_jurnum()
	{
		$ds         = $this->uri->segment('4');
		$de         = $this->uri->segment('5');
		//$period_id  = $this->uri->segment('6');
		...
		// echo  $qLast['last_month'] .'<br>'.  $qLast['yesterday'] .'<br>'. $majors_id;
		...
		// echo json_encode($data['bulanLast']);
		// exit;
		...
		$this->load->view('report/excel_detail_jurnum', $data);
	}

	public function cetak_rekap_jurnum()
	{
		//ob_start();
		...
		$data['bulan']  = $this->Report_model->get_rekap_bulan_jurnum($params);
		$data['free']   = $this->Report_model->get_rekap_bebas_jurnum($params);
		$data['kredit'] = $this->Report_model->get_rekap_jurnal_kredit($params);
		$data['gaji']   = $this->Report_model->get_rekap_gaji_jurnum($params);
		$data['debit']  = $this->Report_model->get_rekap_debit_jurnum($params);
		...
		$data['lap_title'] = 'Tunai';

		// $this->load->library('html2pdflib');
		$this->load->library('dompdflib');

		$filename = 'Rekap Laporan Jurnal per Tanggal ' . pretty_date($ds, 'd-m-Y', FALSE) . ' Sampai ' . pretty_date($de, 'd-m-Y', FALSE) . '.pdf';

		$html = $this->load->view('report/cetak_rekap_jurnum', $data, TRUE);
	...
	public function excel_rekap_jurnum()
	{
		...
		$this->load->view('report/excel_rekap_jurnum', $data);
	}


	public function cetak_detail_kas()
	{
		//ob_start();
		...
		$html = $this->load->view('report/cetak_detail_kas', $data, TRUE);
	...
	public function excel_detail_kas()
	{
		$ds         = $this->uri->segment('4');
		$de         = $this->uri->segment('5');
		$period_id  = $this->uri->segment('6');
		...
		$this->load->view('report/excel_detail_kas', $data);
	}

	public function cetak_rekap_kas()
	{
		//ob_start();
		...
		$html = $this->load->view('report/cetak_rekap_kas', $data, TRUE);
	...
	public function excel_rekap_kas()
	{
		...
		$this->load->view('report/excel_rekap_kas', $data);
	}
	...
	public function excel_rekap_bank()
	{
		...
		if (isset($majors_id) && !empty($majors_id) && $majors_id != 'all') {
			...
			$akunKas = $this->db->query(
				"SELECT account_id FROM account 
				WHERE account.account_category = '2' 
				AND account.account_majors_id = '$majors_id' 
				AND account.account_code LIKE '1%' 
				AND account.account_description LIKE '%Bank%' 
				AND account.account_note IN (SELECT account_id FROM account WHERE account.account_category = '0' 
					AND account.account_majors_id = '$majors_id' 
					AND account.account_code LIKE '1%' 
					AND account.account_description LIKE '%Aktiva%')"
			)->result_array();
	...
	public function cari_kas()
	{
		...
		echo '
			<select class="form-control multiple-select" id="kas_account_id" name="kas_account_id[]" multiple="multiple" required="">';
