				...
				<tr>
					...
					<?php foreach ($bulan as $key) { ?>
						<?php if ($key['student_student_id'] == $row['student_student_id']) { ?>
							<td style="color:<?php echo ($key['bulan_status'] == 1) ? '#00000' : '#00000' ?>"><?php echo ($key['bulan_status'] == 1) ? '-' : number_format($key['bulan_bill'], 0, ',', '.') ?></td>
						<?php } ?>
					<?php } ?>
					<?php foreach ($free as $key) : ?>
						<?php if ($key['student_student_id'] == $row['student_student_id']) { ?>
							<td style="text-align:center;color:<?php echo ($key['bebas_bill'] == $key['bebas_total_pay']) ? '#00000' : '#00000' ?> "><?php echo ($key['bebas_bill'] == $key['bebas_total_pay']) ? '-' : number_format($key['bebas_bill'] - $key['bebas_total_pay'], 0, ',', '.') ?></td>
						<?php } ?>
					<?php endforeach ?>
				</tr>
			<?php endforeach ?>
