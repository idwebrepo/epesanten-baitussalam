...
class Log_trx_model extends CI_Model
{
    ...
    function get_history($params = array())
    {
        ...
        $res = $this->db->query("SELECT DISTINCT DATE(`log_trx_input_date`) AS log_trx_input_date, `bulan_bulan_id`, 
    ...
    function delete_log($params = array())
    {
        ...
        if (isset($params['bulan_id'])) {
            $this->db->where('log_trx.bulan_bulan_id', $params['bulan_id']);
        }

        if (isset($params['bebas_pay_id'])) {
            $this->db->where('log_trx.bebas_pay_bebas_pay_id', $params['bebas_pay_id']);
        }
