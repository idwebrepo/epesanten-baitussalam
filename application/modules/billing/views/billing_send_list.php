					...
					<div class="box box-info">
						<div class="box-body table-responsive">
							<table id="dtable" class="table table-responsive table-bordered" style="white-space: nowrap;">
								...
								<tbody>
									<?php
									$periodActive = $this->db->query("SELECT period_id AS id FROM period WHERE period_status = 1")->row_array;
									$dateM = date('m');
									...
									foreach ($student as $row) :
										...
										$periodNow = $this->db->query("SELECT period_start FROM period WHERE period_id = '" . $q['p'] . "'")->row_array();

										$periodLama = $this->db->query("SELECT period_id FROM period WHERE period_start < '" . $periodNow['period_start'] . "'")->result_array();

										$periode    = NULL;
										foreach ($periodLama as $baris) {
											$periode = $baris['period_id'];
										}
										...
										$paketOld = $this->Billing_model->get_tagihan_paket_lama($param);
									?>
										<tr>
											<td>
											...
											<td>
												...
												foreach ($paketOld as $pakO) {
													$billPaketOld += $pakO['paket_bill'];
												}
												echo 'Rp ' . number_format($billPaket + $billMonth + $billFree + $billMonthOld + $billFreeOld + $billPaketOld, 0, ",", ".");
												?>
											</td>
										</tr>
									<?php endforeach ?>
									...
									<tr style="background-color: #f0f0f0;">
										...
										<td>
											...
											foreach ($pOld as $pOld) {
												$billPOld += $pOld['paket_bill'];
											}

											echo "Rp " . number_format($billM + $billF +  $billP + $billMOld + $billFOld + $billPOld, 0, ",", ".")
											?>
										</td>
									</tr>
								</tbody>
