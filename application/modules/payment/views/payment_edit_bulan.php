							...
							<table class="table">
								<tbody>
									...
									<?php foreach ($bulan as $row): ?>
										<?php
										$status = false;
										if ($row['bulan_bill'] > 0 and $row['bulan_status'] == 1) {
											$status = true;
										}
										?>
										<tr>
											...
											<input type="hidden" name="old_bill[]" value="<?php echo $row['bulan_bill'] ?>">
											<td><input type="text" data-status="<?php echo $status ? 1 : 0  ?>" <?php echo $status ? "readonly" : ""  ?> id="n<?php echo $row['month_month_id'] ?>" name="bulan_bill[]" value="<?php echo $row['bulan_bill'] ?>" class="form-control">
											</td>
										</tr>
...
<script type="text/javascript">
	$("#allTarif").keypress(function(event) {
		...
		if (event.keyCode == 13) {
			...
			<?php foreach ($bulan as $row): ?>
				var status = $("#n<?php echo $row['month_month_id'] ?>").attr('data-status');
				if (status != 1) {
