...
if (isset($payment)) {
	...
	$inputisBatchValue  = $payment['payment_is_batch'];
} else {
	...
	$inputisBatchValue  = set_value('payment_is_batch');
}
?>
			...
			<div class="col-md-9">
				<div class="box box-primary">
					...
					<div class="box-body">
						...
						<div id="div_mode" style="display: none;">
							<div class="form-group">
						...
						<div id="div_is_batch" style="display: none;">
							<div class="form-group">
								<label>Apakah Item Paket Pembayaran<small data-toggle="tooltip" title="Wajib diisi">*</small></label><br>
								<select name="payment_is_batch" id="payment_is_batch" class="form-control" required="">
									<option value="">-Pilih Keterangan-</option>
									<?php if ($paket['val'] == '2' || $paket['val'] == '3') { ?>
										<option value="1" <?php echo ($inputisBatchValue == '1') ? 'selected' : '' ?>>YA</option>
									<?php } ?>
									<option value="0" <?php echo ($inputisBatchValue == '0') ? 'selected' : '' ?>>TIDAK</option>
								</select>
							</div>
						</div>
...
<script>
	...
	function change_cond() {
		...
		var div_is_batch = $("#div_is_batch");
		var div_mode = $("#div_mode");
		...
		if (type === 'BULAN') {
			...
			div_is_batch.hide();
			div_mode.hide();
			$("#payment_is_batch").removeAttr("required");
			$("#payment_mode").removeAttr("required");
		}
	}

	// Call the function when the payment_type changes (assuming it's an input/select element)
