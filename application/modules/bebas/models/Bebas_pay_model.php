...
class Bebas_pay_model extends CI_Model
{
    ...
    function get($params = array())
    {
        ...
        $this->db->select('bebas_pay.bebas_pay_id, bebas_pay_bill, bebas_pay_bill, bebas_pay_noref, bebas_pay_number, bebas_pay_desc, bebas_pay_input_date, bebas_pay_last_update');
    ...
    function get_mutasi($params = array())
    {
        ...
        $this->db->where('majors_status', '1');
        $this->db->distinct();
        $this->db->group_by('bebas_pay_input_date');
        $this->db->group_by('pos_name');

        $this->db->select('bebas_pay.bebas_pay_id, SUM(`bebas_pay_bill`) AS `total_bebas_bill`, bebas_pay_bill, bebas_pay_number, bebas_pay_desc, bebas_pay_input_date, bebas_pay_last_update');
        $this->db->select('bebas_pay.bebas_bebas_id, bebas_bill');
        // $this->db->select_sum('bebas_pay_bill');
        $this->db->select('student_student_id, student_nis, student.class_class_id, class_name, student_full_name, student_name_of_mother, student_parent_phone');
        $this->db->select('payment_payment_id, payment_type, period_start, period_end, payment.pos_pos_id, pos_name');
    ...
    {
        if (isset($params['date_start']) and isset($params['date_end'])) {
            $this->db->where('bebas_pay.bebas_pay_input_date >=', $params['date_start']);
            $this->db->where('bebas_pay.bebas_pay_input_date <=', $params['date_end']);
        }
        ...
        $this->db->select(
            'bebas_pay.bebas_pay_input_date AS tanggal, acc.account_code AS kode_kas, acc.account_description AS nama_kas,
             ap.account_code, pos.pos_name,SUM(bebas_pay.bebas_pay_bill) AS total_bayar'
        );

        $this->db->join('bebas', 'bebas.bebas_id = bebas_pay.bebas_bebas_id', 'inner');
        $this->db->join('account acc', 'acc.account_id = bebas_pay.bebas_pay_account_id', 'inner');
        $this->db->join('payment', 'payment.payment_id = bebas.payment_payment_id', 'inner');
        $this->db->join('pos', 'pos.pos_id = payment.pos_pos_id', 'inner');
        $this->db->join('account ap', 'ap.account_id = pos.account_account_id', 'inner');
        $this->db->where('bebas_pay.bebas_pay_noref IS NOT NULL', null, false);
        $this->db->group_by([
    ...
    function get_last_mutasi($param = array())
    {
        ...
        if (isset($param['kas_account_id'])) {
            $this->db->where_in('bebas_pay.bebas_pay_account_id', $param['kas_account_id']);
        }
    ...
    function get_jurnal($params = array())
    {
        ...
        $this->db->select('account.account_id, account.account_code');
        $this->db->select('acc.account_id, acc.account_code AS kodeKas, acc.account_description');
