...
class Visit_set extends CI_Controller
{
  ...
  public function add()
  {
    if ($_POST == TRUE) {
      ...
      $params['guest_list_note']       = $this->input->post('guest_list_note');
      $params['guest_list_date']       = $this->input->post('guest_list_date');
  ...
  function cetakThermal($id)
  {
    ...
    $student        = $this->db->query("SELECT student_madin from guest_list a left join student b ON b.student_id=a.guest_list_student_id WHERE a.guest_list_id = '$id'")->row_array();

    $check_madin = $student['student_madin'];

    if ($check_madin != 0) {
      $data['visit']   = $this->db->query("SELECT student_nis, student_full_name, class_name, majors_short_name, madin_name, guest_list_id, guest_list_code, guest_list_date, guest_list_time, guest_list_note FROM guest_list JOIN student ON student.student_id = guest_list.guest_list_student_id JOIN class ON class.class_id = student.class_class_id JOIN majors ON majors.majors_id = student.majors_majors_id JOIN madin ON madin.madin_id = student.student_madin WHERE guest_list_id = '$id'")->row_array();
    } else {
      $data['visit']   = $this->db->query("SELECT student_nis, student_full_name, class_name, majors_short_name, guest_list_id, guest_list_code, guest_list_date, guest_list_time, guest_list_note FROM guest_list JOIN student ON student.student_id = guest_list.guest_list_student_id JOIN class ON class.class_id = student.class_class_id JOIN majors ON majors.majors_id = student.majors_majors_id WHERE guest_list_id = '$id'")->row_array();
    }
