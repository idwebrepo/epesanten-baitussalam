...
class Izin_set extends CI_Controller
{
  ...
  public function add()
  {
    if ($_POST == TRUE) {
      ...
      $note = $this->input->post('izin_note');
  ...
  function printBook()
  {
    ...
    $file_pdf = 'Buku_Izin_' . $siswa['student_full_name'] . '_' . date('Y-m-d');
    $paper = 'A4';
    $orientation = "potrait";
  ...
  function cetakThermal($id)
  {
    ...
    $student        = $this->db->query("SELECT student_madin from izin a left join student b ON b.student_id=a.izin_student_id WHERE a.izin_id = '$id'")->row_array();

    $check_madin = $student['student_madin'];

    if ($check_madin != 0) {
      $data['izin']   = $this->db->query("SELECT student_nis, student_full_name, class_name, majors_short_name, madin_name, izin_note, izin_id, izin_date, izin_time FROM izin JOIN student ON student.student_id = izin.izin_student_id JOIN class ON class.class_id = student.class_class_id JOIN majors ON majors.majors_id = student.majors_majors_id JOIN madin ON madin.madin_id = student.student_madin WHERE izin_id = '$id'")->row_array();
    } else {
      $data['izin']   = $this->db->query("SELECT student_nis, student_full_name, class_name, majors_short_name, izin_id, izin_date, izin_time, izin_note FROM izin JOIN student ON student.student_id = izin.izin_student_id JOIN class ON class.class_id = student.class_class_id JOIN majors ON majors.majors_id = student.majors_majors_id WHERE izin_id = '$id'")->row_array();
    }
    ...
    $file_pdf = 'Izin_Keluar_' . $data['student_full_name'] . '_' . date('Y-m-d');
