...
class Halaqoh_pegawai extends CI_Controller {
  ...
  public function index($offset = NULL) {
    ...
    $config['base_url'] = site_url('pegawai/halaqoh/index');
    $config['suffix'] = '?' . http_build_query($_GET, '', "&");
    //$config['total_rows'] = count($this->Student_model->get_halaqoh($paramsPage));
    //$this->pagination->initialize($config);
    $data['majors'] = $this->Student_model->get_majors();
    $data['employee'] = $this->Employees_model->get_employee();
    
    $data['title'] = 'Halaqoh';
    $data['main'] = 'halaqoh/pegawai/halaqoh_list';
  ...
  public function add($id = NULL) {
    ...
    if ($_POST AND $this->form_validation->run() == TRUE) {
      ...
      if ($this->input->post('halaqoh_id')) {
        redirect('pegawai/halaqoh/edit/' . $this->input->post('halaqoh_id'));
      }
      ...
      $data['main'] = 'halaqoh/pegawai/halaqoh_add';
      $this->load->view('pegawai/layout', $data);
    }
  }
...
// activity log
      ...
      redirect('pegawai/halaqoh/pegawai/edit/' . $id);
    }  
  }
  ...
  public function view_setting($id = NULL, $student_id = NULL) {
    ...
    $data['main'] = 'halaqoh/pegawai/halaqoh_view_setting';
    $this->load->view('pegawai/layout', $data);
  }
}
