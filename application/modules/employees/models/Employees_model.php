...
class Employees_model extends CI_Model
{
    ...
    function get($params = array())
    {
        ...
        if (isset($params['position_id'])) {
            $this->db->where('position.position_id', $params['position_id']);
        }
        ...
        $this->db->select("position_id, position_code, position_name, IF(`majors_short_name` IS NULL,'Lainnya',`majors_short_name`) as majors_short_name, majors_name, `position_majors_id`, IF(`majors_id` IS NULL,'99',`majors_id`) as majors_id");
    ...
    function get_employee($params = array()) {
        ...
        if (isset($params['employee_position_id'])) {
            $this->db->where('employee.employee_position_id', $params['employee_position_id']);
            // $this->db->like('employee.employee_position_code', $params['employee_position_id']);
        }
        ...
        if (isset($params['employee_gender'])) {
            $this->db->where('employee.employee_gender', $params['employee_gender']);
        }

        if (isset($params['employee_category'])) {
            $this->db->where('employee.employee_category', $params['employee_category']);
        }

        if (isset($params['employee_strata'])) {
            $this->db->where('employee.employee_strata', $params['employee_strata']);
        }
        ...
        if (isset($params['position_code'])) {
            $this->db->where('position.position_code', $params['position_code']);
        }
        ...
        $this->db->select('employee.employee_id, employee_nip, employee_name, employee_password, employee_email, employee_gender, employee_phone, employee_address, employee_born_place, employee_born_date, employee_photo, employee_status, employee_strata, employee.employee_majors_id, employee_position_id, employee_start, employee_end, employee_category, employee_input_date, employee_last_update');
