...
class Penggajian_set extends CI_Controller {
  ...
  public function set($id = NULL) {
    ...
    if ($_POST) {
      ...
      $gaji_name             = $_POST['gaji_name'];
      $gaji_default_id       = $_POST['gaji_default_id'];
      $gaji_nominal          = $_POST['gaji_nominal'];
      $count_gaji            = count($_POST['gaji_default_id']);
      // var_dump($gaji_setting_id);
      // exit;

      for ($i = 0; $i < $count_gaji; $i++) {
        if(isset($gaji_setting_id[$i])){
          $params['gaji_setting_id']        = $gaji_setting_id[$i];
        }
        $params['gaji_setting_name']        = $gaji_name[$i];
        $params['gaji_setting_default_id']  = $gaji_default_id[$i];
        $params['gaji_setting_nominal']     = $gaji_nominal[$i];
        $params['gaji_setting_employee_id'] = $employee_id;
        // //simpan data setting gaji
        // echo $gaji_setting_id[$i];
        
        // var_dump($params);
        
        $this->Penggajian_model->add_setting_gaji($params);
      ...
      $potongan_name            = $_POST['potongan_name'];
      $potongan_default_id      = $_POST['potongan_default_id'];
      $potongan_nominal         = $_POST['potongan_nominal'];
      $count_potongan           = count($_POST['potongan_default_id']);

      for ($i = 0; $i < $count_potongan; $i++) {
        if(isset($potongan_setting_id[$i]) ){
          $param['potongan_setting_id']         = $potongan_setting_id[$i];
        }
        $param['potongan_setting_name']        = $potongan_name[$i];
        $param['potongan_setting_default_id']  = $potongan_default_id[$i];
        $param['potongan_setting_nominal']     = $potongan_nominal[$i];
        $param['potongan_setting_employee_id'] = $employee_id;
        //simpan data setting potongan
        $this->Penggajian_model->add_potongan_gaji($param);
      }
    ...
    // activity log
      ...
      $this->Logs_model->add(
        array(
          ...
          'log_info' => 'ID:' . $status_akun . ';Name:' . $this->input->post('employee_name')
        )
      );
      ...
      if (!is_null($id)) {
        ...
        $def_penggajian     = $this->Penggajian_model->get_default_gaji(array('default_gaji_mode'=> 0));
        $def_potongan       = $this->Penggajian_model->get_default_gaji(array('default_gaji_mode'=> 1));
        ...
        if ($object == NULL) {
          ...
          $data['def_penggajian'] = $def_penggajian;
          $data['def_potongan']   = $def_potongan;
        }
      }
