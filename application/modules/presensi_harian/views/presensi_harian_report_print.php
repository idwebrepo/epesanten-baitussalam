<?php
$printKelas = "Kelas " . $namaKelas['class_name'];
if (empty($q['c'])) {
	$printKelas = "Semua Kelas";
}

header("Content-type: application/octet-stream");
header("Content-Disposition: attachment; filename=Rekap Presensi " . $majors['majors_short_name'] . " - " . $printKelas . " Bulan " . $namaBulan . " " . $namaTahun . ".xls");
...
<table border="1">
	<thead>
		<tr>
			...
			<th colspan='<?php echo $interval->format("%a") ?>'>
				<center><?php echo $namaBulan . ' ' . $namaTahun ?></center>
			</th>
		</tr>
		<tr>
		...
		?>
			<tr>
				...
				foreach ($daterange as $dt) {
					...
					$period_id = $q['p'];
					$month_id = $q['m'];
					$class_id = empty($q['c']) ? $row['class_id'] : $q['c'];;
