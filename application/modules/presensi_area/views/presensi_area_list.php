					...
					<div class="box-body table-responsive">
						<table>
							<tr>
								...
								<td>
									<input type="submit" name="jarak_radius" id="radius" value="unlock" style="display:none" />
									<button type="button" id="btnRadius" class="btn btn-warning"><i class="fa fa fa-circle-o-notch"></i> Update Radius</button>
								</td>
							</tr>
						</table>
					</div>
...
<div class="modal modal-default fade" id="modalRadius">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span></button>
				<h3 class="modal-title"><span class="fa fa-warning"></span> Update Radius</h3>
			</div>
			<div class="modal-body">
				<p>Masukan Radius Area Presensi!</p>
				<input type="number" id="radius-absen">
				<div id="error-radius" class="error-message" style="color: red; font-size: 12px;"></div>

				<p style="padding-top: 10px;">Pilih Pegawai</p>
				<select id="select-pegawai">
					<option value="">-- Pilih Pegawai--</option>
					<option value="semua">Semua Pegawai</option>
					<option value="cheklist">Sesuai Checklist</option>
				</select>
				<div id="error-pegawai" class="error-message" style="color: red; font-size: 12px;"></div>
			</div>
			<div class="modal-footer">
				<button type="button" id="saveRadius" class="btn btn-warning"><i class="fa fa fa-circle-o-notch"></i> Terapkan Radius</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
...
<script>
	...
	$("#btnRadius").click(function() {
		$("#modalRadius").modal('show');
	});

	$("#saveRadius").click(function() {
		var radius = $("#radius-absen").val();
		var pegawai = $("#select-pegawai").val();

		if (radius == '') {
			document.getElementById('error-radius').textContent = 'Radius tidak boleh kosong';
		} else {
			document.getElementById('error-radius').textContent = '';
		}
		if (pegawai == '') {
			document.getElementById('error-pegawai').textContent = 'Harap pilih pegawai';
		} else {
			document.getElementById('error-pegawai').textContent = '';
		}

		if (radius != '' && pegawai != '') {
			const checkedCheckboxes = document.querySelectorAll('#dtable .checkbox:checked');
			...
			var data = {
				radius: radius,
				pegawai: pegawai,
				list_pegawai: jsonData
			};

			$.ajax({
				url: '<?php echo base_url(); ?>manage/presensi_area/update_radius',
				type: 'POST',
				data: data,
