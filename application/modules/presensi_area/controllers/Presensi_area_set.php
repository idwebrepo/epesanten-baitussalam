...
class Presensi_area_set extends CI_Controller
{
  ...
  function update_radius()
  {
    ...
    $list = $this->input->post('list_pegawai');

    $radiusAbsen = (int)$radius;
    $data['pegawai'] = $pegawai;
    $listPegawai = json_decode($list);

    if ($pegawai == 'semua') {
      $this->db->query("UPDATE employee SET jarak_radius = $radiusAbsen");

      echo 'success';
    } else if ($pegawai == 'cheklist') {
      $placeholders = implode(',', array_fill(0, count($listPegawai), '?'));
      $sql = "UPDATE employee SET jarak_radius = ? WHERE employee_id IN ($placeholders)";
      $params = array_merge([$radiusAbsen], $listPegawai);
