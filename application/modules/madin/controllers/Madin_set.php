...
// Nip
  ...
  public function add_glob(){
    ...
    $this->session->set_flashdata('success',' Tambah Kelas Pondok Berhasil');
...
// Add User_customer and Update
  public function add($id = NULL) {
    ...
    $this->form_validation->set_rules('madin_name', 'Nama Kelas Pondok', 'trim|required|xss_clean');
...
// Edit mode
      ...
      $data['majors'] = $this->Student_model->get_majors();
      $data['title'] = $data['operation'] . ' Keterangan Kelas Pondok';
...
// Delete to database
  public function delete($id = NULL) {
    ...
    if ($_POST) {
      ...
      if (count($siswa) > 0) {
        $this->session->set_flashdata('failed', 'Data Kelas Pondok tidak dapat dihapus');
        redirect('manage/madin');
      }
...
// activity log
      ...
      $this->session->set_flashdata('success', 'Hapus Kelas Pondok berhasil');
      redirect('manage/madin');
    } elseif (!$_POST) {
