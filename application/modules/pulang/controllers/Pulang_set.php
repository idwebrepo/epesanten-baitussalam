...
class Pulang_set extends CI_Controller
{
  ...
  function printBook()
  {
    ...
    if (isset($f['r']) && !empty($f['r']) && $f['r'] != '') {
      ...
      $siswa = $this->Student_model->get(array('student_nis' => $f['r']));
      $siswaID = $siswa['student_id'];
      $namaSantri = $siswa['student_full_name'];
    }
    ...
    $file_pdf = 'Buku_Izin_Pulang_' . $namaSantri . '_' . date('Y-m-d');
    $paper = 'A4';
    $orientation = "potrait";
  ...
  function cetak($id)
  {
    ...
    $student        = $this->db->query("SELECT student_madin from pulang  a left join student b ON b.student_id=a.pulang_student_id WHERE a.pulang_id = '$id'")->row_array();
    ...
    if ($check_madin != 0) {
      ...
      $data['pulang']   = $this->db->query("SELECT student_nis, student_full_name, class_name, majors_short_name, pulang_id, pulang_date, pulang_days, pulang_note FROM pulang JOIN student ON student.student_id = pulang.pulang_student_id JOIN class ON class.class_id = student.class_class_id JOIN majors ON majors.majors_id = student.majors_majors_id WHERE pulang_id = '$id'")->row_array();
    }
    $santri = $data['pulang'];
    $namaSantri = $santri['student_full_name'];
    ...
    $file_pdf = 'Surat_Izin_Pulang_' . $namaSantri . '_' . date('Y-m-d');
    $paper = 'A4';
    $orientation = "potrait";
