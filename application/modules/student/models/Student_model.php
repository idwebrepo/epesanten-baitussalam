...
class Student_model extends CI_Model
{
    ...
    function get($params = array())
    {
        ...
        if (isset($params['halaqoh_id'])) {
            $this->db->where('student.student_halaqoh', $params['halaqoh_id']);
        }
        ...
        $this->db->select('student.student_id, student_nis, student_nisn, student_password, student_gender, student_phone, 
                            ...
                            student_born_date, student_img, student_status, student_halaqoh, student_name_of_mother, student_name_of_father,
        ...
        $this->db->select('halaqoh.halaqoh_id, halaqoh.halaqoh_name, halaqoh.halaqoh_employee_id');

        $this->db->select('student.student_nik, student.student_kk, student.father_nik, student.mother_nik');
        ...
        $this->db->join('halaqoh', 'halaqoh.halaqoh_id = student.student_halaqoh', 'left');
    ...
    function get_halaqoh($params = array())
    {
        if (isset($params['id'])) {
            $this->db->where('halaqoh_id', $params['id']);
        }

        if (isset($params['halaqoh_name'])) {
            $this->db->where('halaqoh_name', $params['halaqoh_name']);
        }

        if (isset($params['halaqoh_employee_id'])) {
            $this->db->where('halaqoh_employee_id', $params['halaqoh_employee_id']);
        }
        ...
        if (isset($params['order_by'])) {
            ...
            $this->db->order_by('halaqoh.halaqoh_name', 'asc');
        }

        // $this->db->where('majors_status', '1');

        $this->db->select('halaqoh_id, halaqoh_name, halaqoh_employee_id');
        $this->db->select('employee_name');
        $this->db->join('employee', 'employee.employee_id=halaqoh.halaqoh_employee_id', 'left');
        $res = $this->db->get('halaqoh');
    ...
    function add_halaqoh($data = array())
    {

        if (isset($data['halaqoh_id'])) {
            $this->db->set('halaqoh_id', $data['halaqoh_id']);
        }

        if (isset($data['halaqoh_name'])) {
            $this->db->set('halaqoh_name', $data['halaqoh_name']);
        }

        if (isset($data['halaqoh_employee_id'])) {
            $this->db->set('halaqoh_employee_id', $data['halaqoh_employee_id']);
        }

        if (isset($data['halaqoh_majors_id'])) {
            $this->db->set('halaqoh_majors_id', $data['halaqoh_majors_id']);
        }

        if (isset($data['halaqoh_id'])) {
            $this->db->where('halaqoh_id', $data['halaqoh_id']);
            $this->db->update('halaqoh');
            $id = $data['halaqoh_id'];
        } else {
            $this->db->insert('halaqoh');
    ...
    function add_halaqoh_relation($data = array())
    {
        if (isset($data['relation_student_id']) && isset($data['relation_halaqoh_id'])) {
            // Periksa apakah data sudah ada dalam tabel halaqoh_relation
            $existingData = $this->db->get_where('halaqoh_relation', array(
                'relation_student_id' => $data['relation_student_id'],
                'relation_halaqoh_id' => $data['relation_halaqoh_id']
            ))->row();

            if (!$existingData) {
                // Jika data tidak ada, maka tambahkan data baru
                if (isset($data['relation_id'])) {
                    // Jika ada 'relation_id', maka ini adalah pembaruan data
                    $this->db->where('relation_id', $data['relation_id']);
                    $this->db->update('halaqoh_relation', $data);
                    $id = $data['relation_id'];
                } else {
                    // Jika tidak ada 'relation_id', maka ini adalah penambahan data baru
                    $this->db->insert('halaqoh_relation', $data);
                    $id = $this->db->insert_id();
                }
            } else {
                // Jika data sudah ada, Anda bisa menambahkan pesan atau tindakan lain sesuai kebutuhan.
                echo 'Data sudah ada';
                $id = $existingData->relation_id;
            }
    ...
    function get_student_halaqoh($params = array())
    {

        if (isset($params['id'])) {
            $this->db->where('halaqoh_relation.relation_id', $params['id']);
        }
        if (isset($params['relation_student_id'])) {
            $this->db->where('halaqoh_relation.relation_student_id', $params['relation_student_id']);
        }

        if (isset($params['student_id'])) {
            $this->db->where('halaqoh_relation.relation_student_id', $params['student_id']);
        }

        if (isset($params['relation_halaqoh_id'])) {
            $this->db->where('halaqoh_relation.relation_halaqoh_id', $params['relation_halaqoh_id']);
        }

        if (isset($params['halaqoh_id'])) {
            $this->db->where('halaqoh_relation.relation_halaqoh_id', $params['halaqoh_id']);
        }
        ...
        $this->db->select('student.student_id, student_nis, student_nisn, student_password, student_gender, student_phone, 
                            ...
                            student_born_date, student_img, student_status, student_name_of_mother, student_name_of_father,
                            student_madin, student_input_date, student_last_update');
        $this->db->select('halaqoh.halaqoh_id, halaqoh.halaqoh_name');
        $this->db->select('halaqoh_relation.relation_id, relation_student_id, relation_halaqoh_id');
        $this->db->select('class.class_id, student.class_class_id, class.class_name, class.majors_majors_id');

        $this->db->join('student', 'student.student_id = halaqoh_relation.relation_student_id', 'left');
        $this->db->join('halaqoh', 'halaqoh.halaqoh_id = halaqoh_relation.relation_halaqoh_id', 'left');
        $this->db->join('class', 'class.class_id = student.class_class_id', 'left');

        $res = $this->db->get('halaqoh_relation');
    ...
    function add($data = array())
    {
        ...
        if (isset($data['father_nik'])) {
            $this->db->set('father_nik', $data['father_nik']);
        }
        ...
        if (isset($data['student_kk'])) {
            $this->db->set('student_kk', $data['student_kk']);
        }
        ...
        if (isset($data['mother_nik'])) {
            $this->db->set('mother_nik', $data['mother_nik']);
        }
        ...
        if (isset($data['student_halaqoh'])) {
            $this->db->set('student_halaqoh', $data['student_halaqoh']);
        }
